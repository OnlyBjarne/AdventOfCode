...
fn main() {
    ...
    let a = one(input.clone());
    let b = two(input.clone());
    println!("a, {}", a);
    println!("b {}", b);
}

fn one(bags: String) -> u32 {
    bags.lines()
        .map(|line| line.split_at(line.len() / 2))
        .map(|(l, r)| [l, r].map(find_unique))
        .map(|[l, r]| u64::trailing_zeros(l & r))
        .sum()
}

fn find_unique(sect: &str) -> u64 {
    sect.bytes()
        .map(|c| match c {
            b'a'..=b'z' => 1 + c - b'a',
            b'A'..=b'Z' => 27 + c - b'A',
...
pub fn two(bags: String) -> u32 {
    bags.lines()
        .array_chunks::<3>() // unstable
        .map(|bags| bags.map(find_unique))
        .map(|[a, b, c]| a & b & c)
